...
GO
----------------------------------------------------------
--changeset kisha.wappnet:23237 
--comment Dec, 2021 6:26:43 AM wappnet/release/1.0.0 

ALTER TABLE CAND_EXAMENES ADD EXAM_START_TIME [datetime] NULL;
ALTER TABLE CAND_EXAMENES ADD EXAM_END_TIME [datetime] NULL;

----------------------------------------------------------
--changeset kisha.wappnet:23238
--comment Mar, 2022 6:26:43 AM wappnet/release/1.0.0  Req # 7 v1.docx
ALTER TABLE CAND_EXAMENES ADD CAND_EXAM_TIME [int] NULL;

----------------------------------------------------------
--changeset kisha.wappnet:23239 
--comment Mar, 2022 6:26:43 AM wappnet/release/1.0.0 Req # 7 v1.docx
ALTER    VIEW [dbo].[CAND_EXAMENES_V2] AS 
SELECT NUMERO
	, NUMERO_USUARIO
 ...
 AND   CCE.NUMERO = CE.[NUMERO_EXAMEN] AND AD.NUMERO = AURV1.NUMERO_USUARIO
   ...
   GROUP BY NUMERO, NUMERO_USUARIO, NUMERO_EXAMEN, DESCRIPCION, TITULO, FECHA_EFECTIVA_DESDE_EXAMEN
	, FECHA_EFECTIVA_HASTA_EXAMEN, TIPO, ESTATUS, CREADO_POR,FECHA_CREACION
 ,ACTUALIZADO_POR, FECHA_ACTUALIZACION, MATRICULA, NOMBRE_USUARIO, APELLIDO_PATERNO, APELLIDO_MATERNO
 ,NOMBRE_COMPLETO_USUARIO, NOMBRE_ROL, DESCRIPCION_ROL, CURP, TIEMPO_LIMITE,  TOTAL_PUNTUACION, CAND_EXAM_TIME, NOMBRE_ACT_POR
...
SET QUOTED_IDENTIFIER ON
GO
ALTER VIEW [dbo].[CAND_EXAMENES_V1] AS
SELECT CE.[NUMERO]
,CE.[NUMERO_USUARIO]
,CE.[NUMERO_EXAMEN]
,EX.DESCRIPCION
...
FROM [dbo].[CAND_EXAMENES] CE
,[dbo].[ADMON_USUARIOS_ROLES_V1] AURV1
,[dbo].[MRQS_EXAMENES] EX
,[dbo].ADMON_USUARIOS AD
WHERE CE.NUMERO_USUARIO = AURV1.NUMERO_USUARIO
AND EX.NUMERO = CE.[NUMERO_EXAMEN] AND AD.NUMERO = AURV1.NUMERO_USUARIO
GO
--------------------------------------